...
namespace GeometryElements
{
    ...
    public abstract class Element
    {
        ...
        public Element MoveCopy(Point3D pt) => MoveCopy(pt.X, pt.Y, pt.Z);
        /// <summary>Copie puis déplace le nouvel élément relativement à sa position actuelle.</summary>
    ...
    public class Polygon : Element
    {
        ...
        public virtual List<Point3D> Points => Shape.MoveRotateCopy(Position, Orientation);

        public override string ToString() => $"Position {Position} | {Shape.Count} points | IsFilled: {IsFilled}";
        public override Element Copy()
        {
            List<Point3D> newShape = new List<Point3D>();
            foreach (Point3D point in Shape)
                newShape.Add(point.Copy());

            return new Polygon(newShape, Position.Copy(), Color, Name)
            {
                IsFilled = IsFilled
            };
        }
        ...
        public PointList GetBorderPointList(PointList.PointListType type = PointList.PointListType.Loop)
            => GetBorderPointList(Color.Green, type);
        /// <summary>Retourne la liste de points définissant le polygone en tout instant.</summary>
    ...
    public class ElementList : Element
    {
        ...
        public override Element Copy()
        {
            List<Element> newShape = new List<Element>();
            foreach (Element element in Shape)
                newShape.Add(element.Copy());

            return new ElementList(newShape, Position.Copy(), Color, Name) { Orientation = Orientation.Copy() };
        }
    }
    ...
    public class Segment : Element
    {
        ...
        /// <summary>Liste de points placés et orientés selon Position et Orientation.</summary>
        public virtual List<Point3D> Points => new List<Point3D> { A, B }.MoveRotateCopy(Position, Orientation);

        public override string ToString() => $"Position {Position} | A: {A} -> B: {B}";
        public override Element Copy() => new Segment(A.Copy(), B.Copy(), Position.Copy(), Color, Name) { Orientation = Orientation.Copy() };
    }
    ...
    public class Triangle : Element
    {
        ...
        /// <summary>Liste de points placés et orientés selon Position et Orientation.</summary>
        public virtual List<Point3D> Points => new List<Point3D> { A, B, C }.MoveRotateCopy(Position, Orientation);

        public override string ToString() => $"Position {Position} | A: {A} -> B: {B} -> C: {C}";
        public override Element Copy() => new Triangle(A.Copy(), B.Copy(), C.Copy(), Position.Copy(), Color, Name) { Orientation = Orientation.Copy() };
    }
    ...
    public class PointList : Element
    {
        ...
        /// <summary>Liste de points placés et orientés selon Position et Orientation.</summary>
        public virtual List<Point3D> Points => SourcePoints.MoveRotateCopy(Position, Orientation);
    ...
    public class OutlinedRectangle : ElementList
    {
        ...
        public override Element Copy() => new OutlinedRectangle(Width, Height, Thickness, Position.Copy(), Color, Name) { Orientation = Orientation.Copy() };
    }
    ...
    public class OutlinedCircle : Circle
    {
        ...
        public OutlinedCircle(double radius, double thickness) => Thickness = thickness;
    ...
    public class Arrow : ElementList
    {
        ...
        public double InnerLength { get; set; } = 0;
        ...
        public Arrow(double length, double height, double innerLength, Point3D position, Point3D orientation, Color color, string name) : base(null, position, color, name)
        {
            ...
            Shape = new List<Element>
            {
                new Rectangle(Length - InnerLength, Height * 0.2, new Point3D(-InnerLength / 2, 0), color, name + "_base"),
                new Triangle(new Point3D(0, 0),
                             new Point3D(0, Height), 
                             new Point3D(InnerLength, Height / 2), 
                             new Point3D(Length / 2 - InnerLength, -Height / 2), color, name + "_tip")
            };
        }
    ...
    public class BigArrow : ElementList
    {
        ...
        public override string ToString() => $"Position: {Position} | Length: {Length} | Height: {Height} | InnerLength: {InnerLength}";
        public override Element Copy() => new BigArrow(Length, Height, InnerLength, Position.Copy(), Color, Name) { Orientation = Orientation.Copy() };
    }
    ...
    public class Mark : ElementList
    {
        ...
        public override Element Copy() => new Mark(Width, Height, Thickness, Position.Copy(), Color, Name) { Orientation = Orientation.Copy() };
    }
